...
3. 更新总 README 的组件、版本和用途；在组件 README 明确 MCU/HAL/器件支持范围、依赖和已完成的验证。
...
- 存储驱动将通用操作、器件参数和 HAL 平台实现分开。应用通过 `get_info` 查询容量与能力；新增型号需核对协议与时序并补充验证，不能只改型号或 JEDEC ID。未实现的总线明确返回不支持。
